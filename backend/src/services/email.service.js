...
export async function sendDeviceSearchedAlert(ownerEmail, ownerName, deviceInfo, searcherInfo) {
  await getResend().emails.send({
    from:    FROM(),
    to:      ownerEmail,
    subject: '⚠️ تنبيه: تم البحث عن جهازك',
    html: `<div dir="rtl" style="font-family:Arial;max-width:480px;margin:auto">
      <h2 style="color:#C0392B">تنبيه هام</h2>
      <p>مرحباً ${ownerName || ''},</p>
      <p>قام شخص ما بالبحث عن جهازك على منصة CheckMyDevice.</p>
      <div style="background:#FEF9E7;border-right:4px solid #E67E22;padding:12px;margin:16px 0">
        <strong>الجهاز:</strong> ${deviceInfo.brand} ${deviceInfo.model}<br>
...
export async function sendReportStatusEmail(email, name, status, reportRef, adminNote) {
  ...
  await getResend().emails.send({
    ...
    html: `<div dir="rtl" style="font-family:Arial;max-width:480px;margin:auto">
      ...
      <p>${isApproved
        ? 'تم إضافة جهازك إلى قاعدة بياناتنا.'
        : 'إذا كنت تعتقد أن هذا قرار خاطئ، تواصل مع الدعم.'
      }</p>
    </div>`,
  });
}
